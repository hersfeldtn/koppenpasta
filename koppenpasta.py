...
Load config file? (y/n): ''')
        if cfg_op in ('y') or cfg_op in ('1'):
            ...
            col_list = typ.get('Custom Color List')
            if color_type == 3:
                while True:
...
Set Color List: '''))
            ...
            interp = adv.getint('Interpolation Runs')
...
Note that:
    The configs do not include the input or output filenames, nor the topography map for interpolation
...
Save configuration to config file? (y/n): ''')
            if cfg_save_op in ('y') or cfg_save_op in ('1'):
                ...
                config['Zone Types'] = {'Land Climate Zones': str(land_type),
                                        ...
                                        'Custom Color List': str(col_list)}
                config['Advanced Functions'] = {'Bin Size': str(bin_num),
                                                'Interpolation Runs': str(interp),
...
''')
    ...
    if color_type == 3:
        print('Loading Custom Color List...')
        cfg = configparser.ConfigParser()
    ...
    #Applies that function to our climate zone arrays and draws images from them
    print('Drawing Climate Maps...')
    if blend == 1:
        Kop_rgb = []
        color(Koppen_Full, Kop_rgb)
...
Operation Complete""")
    if blend == 1:
        if sea_type == 5:
            print ('Map Written to ' + str(path) + str(output_name) + '_land.png')
        else:
            print ('Maps Written to ' + str(path) + str(output_name) + '_land.png')
